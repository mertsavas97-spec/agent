...
| 6 | Delete request soft-flag + optional hard purge of own data | PASS | `deleteRequestedAt` then `purgeAccount` (own uid only) |
...
- Hard purge callable shipped (`purgeAccount`); scheduled ops sweep still optional.

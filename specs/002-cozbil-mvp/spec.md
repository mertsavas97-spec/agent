...
**Input**: Owner product brief + MVP moodboard — multi-exam (LGS, YGS, KPSS) AI photo question solver for Türkiye, Android-first MVP 1.0.
...
İlk açılışta 3 ekranlı onboarding (moodboard): (1) fotoğrafla çöz, (2) adım adım anlatır, (3) “Hangi sınava hazırlanıyorsun?” — **LGS, YGS ve KPSS seçilebilir ve aktiftir**. Yaşa uygun KVKK/veli onayı: özellikle LGS / reşit olmayan kullanıcılar için; yetişkin KPSS/YGS adaylarında standart aydınlatma + onay.

**Why this priority**: Çoklu sınav konumlandırmasını ve güven algısını ilk dakikada kurar.

**Independent Test**: Yeni kullanıcı LGS, YGS veya KPSS seçerek Ana Sayfa’ya ulaşabilir; üç seçenek de disabled değildir.
...
Ücretsiz kullanıcı günde sınırlı soru hakkına sahiptir (varsayılan **5**/gün). Hak bitince paywall (moodboard Premium): sınırsız soru, reklamsız, kişisel AI analizi vb.; CTA “Hemen Başla”; planlar: **14,90 TL / 7 gün** giriş, **39 TL / ay**, **349 TL / yıl** (bkz. `docs/product/pricing-policy.md`). Ücretsiz modda reklamlar `docs/product/ads-policy.md` matrisine göre (banner tab’larda; günde ≤1 interstitial çözüm çıkışında 3+ çözümeden sonra; ödüllü reklam paywall’da +1 hak, zorunlu değil). Premium’un **üç planında da** reklam kapalı.
...
- **FR-013**: 3 adımlı onboarding + **LGS, YGS, KPSS** aktif sınav seçimi sunulmalı (hiçbiri disabled/yakında değil).
...
- **Mini özgün item bank** (~50–60 madde): LGS / YGS (LYS hattı) / KPSS tarzında
...
- **StudentAccount** (User): Kimlik, examType (`lgs`|`ygs`|`kpss`), yaş/onay, streak, kota, abonelik.
...
- **SC-003**: Onboarding’de LGS, YGS ve KPSS seçen test kullanıcılarının her biri Ana Sayfa’ya ulaşır (üç yol da yeşil).
...
- **SC-006**: Lansman mesajlaşması LGS+YGS+KPSS kapsamını doğru yansıtır; “yalnız LGS” iddiası taşımaz.

## Assumptions

- Working brand **ÇözBil**; mağaza/alan adı owner doğrulaması bekler.
- Ücretsiz günlük hak **5**; Premium: haftalık giriş **14,90 TL**, aylık **39 TL**, yıllık **349 TL**.
...
- MVP konu kataloğu: her sınav için matematik-öncelikli iskelet; YGS/KPSS katalogları LGS’ten ayrı dosyalarda genişletilir.

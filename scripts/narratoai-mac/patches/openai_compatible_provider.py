...
def _vertex_adc_access_token() -> str:
    """Refresh Google ADC token for Vertex OpenAI-compatible endpoint (Startup credits)."""
    try:
        import google.auth
        from google.auth.transport.requests import Request
    except ImportError as exc:
        raise ConfigurationError(
            "google-auth is required for Vertex ADC. Run: uv add google-auth",
            "api_key",
        ) from exc
    ...
    if not getattr(credentials, "token", None):
        raise AuthenticationError(
            "Vertex ADC returned empty token. Run: gcloud auth application-default login"
        )
...
class _OpenAICompatibleBase:
    ...
    def _build_client(
        ...
        # Startup / GFS credits: Vertex OpenAI-compatible + ADC (same project as MPT)
        if config.app.get("vertex_use_adc") or str(config.app.get("gemini_use_vertex", "")).lower() in {
            "1", "true", "yes", "on",
        }:
            api_key = _vertex_adc_access_token()
            project = (
                config.app.get("gemini_vertex_project")
                or config.app.get("vertex_project")
                or ""
            )
            location = (
                config.app.get("gemini_vertex_location")
                or config.app.get("vertex_location")
                or "us-central1"
            )
            if project and (not base_url or "aiplatform.googleapis.com" not in str(base_url)):
                base_url = (

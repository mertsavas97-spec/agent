#!/usr/bin/env bash
# Mac: apps/mobile/.env.local yaz (Firebase key gcloud'dan — chat/PR/commit yok)
#
...
# Yeni client key (eski Browser key silindi)
...
# Some gcloud versions print "keyString: AIza..." — strip label
KEY="${KEY#keyString: }"
...
umask 077
cat >"$OUT" <<EOF
# Generated by scripts/write-mobile-env-local.sh — DO NOT COMMIT
...
echo ""
echo "Sonraki:"
echo "  bash scripts/phone-demo-mac.sh fix-backend"

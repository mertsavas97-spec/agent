...
def _serialize_materials_for_state(downloaded_videos):
    """State/API payloads must be JSON-friendly (no BrainrotMaterials objects)."""
    if isinstance(downloaded_videos, brainrot.BrainrotMaterials):
        return {
            "mode": "brainrot",
            "background": downloaded_videos.background_path,
            "overlays": list(downloaded_videos.overlay_paths or []),
            "bg_pack": downloaded_videos.bg_pack,
            "bg_query": downloaded_videos.bg_query,
        }
    return downloaded_videos


def get_video_materials(task_id, params, video_terms, audio_duration):
    render_mode = str(getattr(params, "render_mode", None) or "pexels").lower()
    if render_mode == "brainrot":
        logger.info("\n\n## preparing brainrot materials (Pexels BG + PiP overlays)")
        materials = brainrot.prepare_brainrot_materials(
            task_id=task_id,
            params=params,
        ...
        if not materials:
            _mark_task_failed(
                task_id,
                "materials",
                "failed to prepare brainrot background/overlays from Pexels",
            )
            return None
        return materials
...
def generate_final_videos(
    ...
    for i in range(params.video_count):
        ...
        if isinstance(downloaded_videos, brainrot.BrainrotMaterials):
            brainrot.apply_brainrot_subtitle_defaults(params)
            brainrot.combine_brainrot_video(
...
def _run_pipeline(
    ...
    if stop_at == "materials":
        materials_payload = _serialize_materials_for_state(downloaded_videos)
        sm.state.update_task(
            task_id,
            state=const.TASK_STATE_COMPLETE,
            progress=100,

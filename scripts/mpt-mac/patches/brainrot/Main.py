...
def _render_video_settings(panel, params):
    ...
    with panel:
        with st.container(border=True):
            ...
            render_modes = [
                (tr("Classic B-roll (Pexels cutaways)"), "pexels"),
                (tr("Brainrot (BG loop + PiP overlays)"), "brainrot"),
            ]
            saved_render_mode = config.app.get("render_mode", "pexels")
            params.render_mode = stable_selectbox(
                tr("Render Mode"),
                options=[value for _, value in render_modes],
                default_value=saved_render_mode,
                key="render_mode_select",
                format_func=lambda value: dict((v, label) for label, v in render_modes)[
                    value
                ],
            )
            config.app["render_mode"] = params.render_mode

            if params.render_mode == "brainrot":
                st.caption(
                    tr(
                        "Brainrot: fullscreen muted satisfaction/abstract BG from Pexels; "
                        "topic photos as PiP (never fullscreen) so the BG stays visible."
                    )
                )
                bg_packs = [
                    ("Satisfaction (sand/slime/pour)", "satisfaction"),
                    ("Abstract motion", "abstract"),
                    ("Nature motion", "nature_motion"),
                    ("Mechanica", "mechanica"),
                ]
                saved_pack = config.app.get("brainrot_bg_pack", "satisfaction")
                params.brainrot_bg_pack = stable_selectbox(
                    tr("Brainrot BG Pack"),
                    options=[v for _, v in bg_packs],
                    default_value=saved_pack,
                    key="brainrot_bg_pack_select",
                    format_func=lambda value: dict((v, label) for label, v in bg_packs)[
                        value
                    ],
                )
                config.app["brainrot_bg_pack"] = params.brainrot_bg_pack

                params.brainrot_overlay_count = st.slider(
                    tr("PiP overlay image count"),
                    min_value=0,
                    max_value=10,
                    value=int(config.app.get("brainrot_overlay_count", 5) or 5),
                    key="brainrot_overlay_count_slider",
                )
                config.app["brainrot_overlay_count"] = params.brainrot_overlay_count

                params.brainrot_overlay_max_width_pct = st.slider(
                    tr("PiP max width %"),
                    min_value=30,
                    max_value=80,
                    value=int(
                        float(config.app.get("brainrot_overlay_max_width_pct", 0.64))
                        * 100
                    ),
                    key="brainrot_overlay_width_slider",
                ) / 100.0
                config.app["brainrot_overlay_max_width_pct"] = (
                    params.brainrot_overlay_max_width_pct
                )

                params.brainrot_overlay_max_height_pct = st.slider(
                    tr("PiP max height %"),
                    min_value=20,
                    max_value=60,
                    value=int(
                        float(config.app.get("brainrot_overlay_max_height_pct", 0.42))
                        * 100
                    ),
                    key="brainrot_overlay_height_slider",
                ) / 100.0
                config.app["brainrot_overlay_max_height_pct"] = (
                    params.brainrot_overlay_max_height_pct
                )

                params.brainrot_overlay_max_coverage = st.slider(
                    tr("PiP max frame coverage %"),
                    min_value=20,
                    max_value=70,
                    value=int(
                        float(config.app.get("brainrot_overlay_max_coverage", 0.55))
                        * 100
                    ),
                    key="brainrot_overlay_coverage_slider",
                ) / 100.0
                config.app["brainrot_overlay_max_coverage"] = (
                    params.brainrot_overlay_max_coverage
                )

                positions = [
                    ("Center upper (recommended)", "center_upper"),
                    ("Center", "center"),
                    ("Top", "top"),
                ]
                saved_pos = config.app.get(
                    "brainrot_overlay_position", "center_upper"
                )
                params.brainrot_overlay_position = stable_selectbox(
                    tr("PiP position"),
                    options=[v for _, v in positions],
                    default_value=saved_pos,
                    key="brainrot_overlay_position_select",
                    format_func=lambda value: dict(
                        (v, label) for label, v in positions
                    )[value],
                )
                config.app["brainrot_overlay_position"] = (
                    params.brainrot_overlay_position
                )
...
def _render_generation_controls(
    ...
    if start_button:
        ...
        if (
            ...
            or str(getattr(params, "render_mode", "")) == "brainrot"
        ) and not config.app.get(
            "pexels_api_keys", ""
        ):
            _remove_active_generation_task(task_id)

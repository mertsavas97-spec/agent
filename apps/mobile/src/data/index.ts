...
import { TRAFIK_TOPICS } from './trafik-topics';
import { YGS_TOPICS } from './ygs-topics';

export type { Topic } from './topics';
export { KPSS_TOPICS, LGS_TOPICS, TRAFIK_TOPICS, YGS_TOPICS };
export {
  ALL_KNOWN_SUBJECTS,
  SUBJECT_LABEL,
  isKnownSubject,
...
export function findTopic(topicId: string): Topic | undefined {
  return [...LGS_TOPICS, ...YGS_TOPICS, ...KPSS_TOPICS, ...TRAFIK_TOPICS].find(
    (t) => t.id === topicId,
  );
}
